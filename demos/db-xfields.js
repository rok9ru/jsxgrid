(function() {
    ...
    db.clients = [
        ...
        },
        {
            ...
            "Age": 59,
            "Country": 1,
            "Address": "967-5176 Tincidunt Avdfghdhdfghdfghdfghdfghdfghdfghrte 45 634 563 3 635 634 563 65.",
            "Married": false
        },
        {
            "Name": "Keely Silva",
    ...
    // Backfill img/date for every row so the demo grid looks populated,
    // while leaving a few rows sparse on purpose (shows the fields degrade
    // gracefully on missing data, e.g. XDateTimeField on an empty value).
    db.clients.forEach(function (client, index) {
        if (!client.img && index % 13 !== 0) {
            client.img = 'https://i.pravatar.cc/80?img=' + ((index % 70) + 1);
        }
        if (!client.date && index % 15 !== 0) {
            var start = Date.UTC(2023, 0, 1);
            var span = 3 * 365 * 24 * 60 * 60 * 1000;
            var stamp = new Date(start + ((index * 100249) % span));
            client.date = stamp.toISOString().slice(0, 19).replace('T', ' ');
        }
    });
